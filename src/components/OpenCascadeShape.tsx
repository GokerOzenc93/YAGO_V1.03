...
const OpenCascadeShape: React.FC<Props> = ({
  ...
  const handleClick = (e: any) => {
    ...
    if (isFaceEditMode && e.nativeEvent.button === 0) {
      ...
      let seq = faceSequenceRef.current;

      const samePoint =
        seq && Math.abs(seq.x - clientX) < 2 && Math.abs(seq.y - clientY) < 2;
      ...
      if (!samePoint) {
        ...
        seq = { x: clientX, y: clientY, hits, index: 0 };
      }

      const hit = seq!.hits[seq!.index];
      seq!.index = (seq!.index + 1) % seq!.hits.length;
      faceSequenceRef.current = seq;

      const highlight = highlightFace(scene, hit, shape, 0xff6b35, 0.6, true);

      if (highlight && onFaceSelect && hit.faceIndex !== undefined) {
